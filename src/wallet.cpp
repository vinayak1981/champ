...
// champcoin: optional setting to unlock wallet for block minting only;
...
// champcoin: total coins staked (non-spendable until maturity)
...
bool CWallet::SelectCoinsMinConf(int64 nTargetValue, unsigned int nSpendTime, int nConfMine, int nConfTheirs, vector<COutput> vCoins, set<pair<const CWalletTx*,unsigned int> >& setCoinsRet, int64& nValueRet) const
{
    ...
    BOOST_FOREACH(COutput output, vCoins)
    {
        ...
        if (pcoin->nTime > nSpendTime)
            continue;  // champcoin: timestamp must not exceed spend time
...

    {
        ...
        CTxDB txdb("r");
        {
            ...
            while(true)
            {
                ...
                // champcoin: sub-cent change is moved to fee
...
// champcoin: create coin stake transaction
...
// champcoin: check 'spent' consistency between wallet and txindex
// champcoin: fix wallet spent state according to txindex
...
// champcoin: disable transaction (only for coinstake)
